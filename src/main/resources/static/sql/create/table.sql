...
(
select *, 1 as tableCounts, 1 as flowCounts, 1 as searchCounts, 1 as chartsCounts
from module_type);
-- 模块应用查看视图
...
(
    ...
    `titleColumnId`       BIGINT(64) UNSIGNED                NULL COMMENT '流程标题字段id',
...
(
select r.*, request_done.humanId, request_done.doneTime, request_done.nodeId
from `request_done`
         left join request r on request_done.requestId = r.requestId
    );

create view `request_back_log_view` as
(
select r.*, request_backlog.humanId, request_backlog.arriveTime, request_backlog.nodeId
from `request_backlog`
...
DELIMITER $$
USE `eoa_build`$$
CREATE DEFINER = CURRENT_USER TRIGGER `eoa_build`.`page_config_BEFORE_INSERT` BEFORE INSERT ON `page_config` FOR EACH ROW
BEGIN
    if new.onUse = 1 then
        update page_config set onUse = 0;
    end if;
END$$
DELIMITER ;
...
DELIMITER $$
USE `eoa_build`$$
CREATE DEFINER = CURRENT_USER TRIGGER `eoa_build`.`page_config_BEFORE_UPDATE` BEFORE UPDATE ON `page_config` FOR EACH ROW
BEGIN
    if new.onUse = 1 then
        update page_config set onUse = 0;
    end if;
...
DELIMITER $$
USE `eoa_build`$$
CREATE DEFINER = CURRENT_USER TRIGGER `eoa_build`.`login_config_BEFORE_INSERT` BEFORE INSERT ON `login_config` FOR EACH ROW
BEGIN
    if new.onUse = 1 then
        update login_config set onUse = 0;
    end if;
END$$
DELIMITER ;
...
DELIMITER $$
USE `eoa_build`$$
CREATE DEFINER = CURRENT_USER TRIGGER `eoa_build`.`login_config_BEFORE_UPDATE` BEFORE UPDATE ON `login_config` FOR EACH ROW
BEGIN
    if new.onUse = 1 then
        update login_config set onUse = 0;
    end if;
